"""
Bundesliga 25/26 이적 데이터 수집 — worldfootball.net 기반
...
출력:
    transfer_news_bundesliga_bl_2526.sql
"""
...
BOT_HANDLE  = "bl_import_bot"
BOT_NAME    = "Bundesliga Import Bot"
POST_PREFIX = "WF_BL"
OUT_SQL     = os.path.join(os.path.dirname(__file__), "transfer_news_bundesliga_bl_2526.sql")

WF_BASE       = "https://www.worldfootball.net"
WF_SLUG       = "bundesliga-2025-2026"
REQUEST_DELAY = 2.0
...
def parse_fee(fee_text: str) -> tuple[int | None, str]:
    ...
    if any(w in t for w in ("loan", "leihe", "lend", "ausleihe")):
        return None, "LOAN"
    if any(w in t for w in ("free", "ablösefrei", "bosman", "ohne ablöse")):
        return 0, "CONFIRMED"
    m = re.search(r"[\d.,]+", t)
    if not m:
        return None, "CONFIRMED"
...
def _parse_date_month(date_str: str) -> int | None:
    """'15.07.2025' or '07/15/2025' 형식에서 월 추출"""
...
def _parse_row(cells: list) -> dict | None:
    """table row의 td 목록에서 이적 정보 파싱"""
    if len(cells) < 5:
        return None

    # 선수 이름: /spieler_profil/ 링크
    player_link = None
    for cell in cells:
