"""
Ligue 1 25/26 이적 데이터 수집 — worldfootball.net 기반
...
출력:
    transfer_news_ligue1_l1_2526.sql
"""
...
DB_SEASON   = 51
LEAGUE_NAME = "Ligue 1"
BOT_HANDLE  = "l1_import_bot"
BOT_NAME    = "Ligue 1 Import Bot"
POST_PREFIX = "WF_L1"
OUT_SQL     = os.path.join(os.path.dirname(__file__), "transfer_news_ligue1_l1_2526.sql")
...
def parse_fee(fee_text: str) -> tuple[int | None, str]:
    ...
    if any(w in t for w in ("loan", "leihe", "lend", "prêt", "ausleihe")):
        return None, "LOAN"
    if any(w in t for w in ("free", "ablösefrei", "bosman", "libre")):
        return 0, "CONFIRMED"
    m = re.search(r"[\d.,]+", t)
    if not m:
        return None, "CONFIRMED"
